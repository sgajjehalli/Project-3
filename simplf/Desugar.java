...
public class Desugar implements Expr.Visitor<Expr>, Stmt.Visitor<Stmt> {
    ...
    public Stmt visitForStmt(For stmt) {
        List<Stmt> forBlockStatements = new ArrayList<>();

        // Initialize variable if present
        if (stmt.init != null) {
            forBlockStatements.add(new Expression(stmt.init));
        }

        // Prepare the body of the while loop, including the for loop body and increment if present
        List<Stmt> whileBodyStatements = new ArrayList<>();
        whileBodyStatements.add(stmt.body);
        if (stmt.incr != null) {
            whileBodyStatements.add(new Expression(stmt.incr));
        }
        Stmt.Block whileBody = new Block(whileBodyStatements);

        // Create the equivalent while loop using the condition
        Stmt whileLoop = new While(stmt.cond == null ? new Literal(true) : stmt.cond, whileBody);

        // Add the while loop to the block
        forBlockStatements.add(whileLoop);

        // Return a new block with the desugared for-loop contents
        return new Block(forBlockStatements);
    }
